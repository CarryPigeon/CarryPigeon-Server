...
import team.carrypigeon.backend.connectionpool.protocol.encryption.aes.AESData;
...
public class NettySession implements CPSession {
    ...
    private void init(){
        ...
        // 添加时间戳
        attributes.put(ConnectionAttributes.PACKAGE_TIMESTAMP, System.currentTimeMillis());
    }
    ...
    public void write(String msg, boolean encrypted) {
        if(encrypted){
            ...
            // 填充包序列
            System.arraycopy(attributes.get(ConnectionAttributes.PACKAGE_ID).toString().getBytes(),0,aad,0,4);
            // 填充会话id
            System.arraycopy(attributes.get(ConnectionAttributes.PACKAGE_SESSION_ID).toString().getBytes(),0,aad,4,12);
            // 填充时间戳
            System.arraycopy(attributes.get(ConnectionAttributes.PACKAGE_TIMESTAMP).toString().getBytes(),0,aad,16,4);
            // 创建密文
            AESData aesData = null;
            try {
            ...
            context.writeAndFlush(msg.getBytes());
        }
