...
## Actual Result
- This sheet remained a planning artifact and was not executed as an isolated implementation batch.
- During the later `docs/t` alignment work, controller annotations, DTO schemas, OpenAPI config, and `docs/API.md` were substantially updated as part of real protocol remediation work.
- The repository now has broad `@Tag` / `@Operation` / `@ApiResponses` / `@Schema` coverage across the active HTTP controller surface, and `application-starter` already contains `OpenApiConfigurationTests` verifying the portal baseline.

## Verification Record
- `application-starter/src/test/java/team/carrypigeon/backend/starter/config/OpenApiConfigurationTests.java` exists and verifies OpenAPI bean registration and security tagging baseline.
- Current controller surfaces under `chat-domain/.../controller/http/` already carry non-trivial OpenAPI annotations and DTO schema metadata.
- `docs/API.md` has been updated during the broader API alignment work and no longer blocks protocol-level client integration.

## Residual Risk
- Further Swagger/UI polish is still possible, but it is no longer a protocol-blocking or docs/t-alignment-blocking task.
- If a future effort wants endpoint-by-endpoint documentation enrichment beyond current coverage, it should be reopened as a dedicated docs-only refinement task, not kept as an ambiguous `current` artifact.
