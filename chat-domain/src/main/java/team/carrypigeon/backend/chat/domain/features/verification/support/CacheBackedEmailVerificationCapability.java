...
/**
 * 基于缓存的邮箱验证码 capability 实现。
 * 职责：把邮箱验证码持久化到共享缓存，并完成验证码邮件投递编排。
 * 边界：缓存与邮件客户端的具体实现细节停留在基础设施层。
 */
public class CacheBackedEmailVerificationCapability implements EmailVerificationCapability {
    ...
    public CacheBackedEmailVerificationCapability(CacheService cacheService) {
        this(cacheService, null);
    }

    public CacheBackedEmailVerificationCapability(CacheService cacheService, MailSenderService mailSenderService) {
