...
@Slf4j
@AllArgsConstructor
@LiteflowComponent("ApiChannelCreateBind")
public class ApiChannelCreateBindNode extends CPNodeComponent {
    ...
    protected void process(CPFlowContext context) {
        ...
        long cid = IdUtil.generateId();
        long createTimeMillis = TimeUtil.currentTimeMillis();

        long avatarId = resolveAvatarId(req.avatar(), ownerUid);
        String brief = req.brief() == null ? "" : req.brief();

        context.set(CPNodeChannelKeys.CHANNEL_INFO_ID, cid);
        context.set(CPNodeChannelKeys.CHANNEL_INFO_NAME, req.name());
        context.set(CPNodeChannelKeys.CHANNEL_INFO_OWNER, ownerUid);
        context.set(CPNodeChannelKeys.CHANNEL_INFO_BRIEF, brief);
        context.set(CPNodeChannelKeys.CHANNEL_INFO_AVATAR, avatarId);
        context.set(CPNodeChannelKeys.CHANNEL_INFO_CREATE_TIME, createTimeMillis);
