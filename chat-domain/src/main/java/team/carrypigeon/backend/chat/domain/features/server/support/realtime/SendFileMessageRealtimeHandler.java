...
/**
 * 文件消息实时入站处理器。
 * 职责：解析统一 realtime 文件消息命令并委托消息应用服务发送。
 * 边界：只负责协议入站到草稿对象的转换，不承载文件消息业务规则。
 */
public class SendFileMessageRealtimeHandler implements RealtimeInboundMessageHandler {

    private static final String GENERIC_SEND_COMMAND = "send_channel_message";

    private final JsonProvider jsonProvider;

    public SendFileMessageRealtimeHandler(JsonProvider jsonProvider) {
    ...
    public boolean supports(RealtimeClientMessage request) {
        return request != null
                ...
                && "file".equals(request.messageType());
    }
