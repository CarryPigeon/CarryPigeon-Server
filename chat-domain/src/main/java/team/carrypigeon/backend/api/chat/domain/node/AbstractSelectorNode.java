 ...
 *   <li>查询结果为 null 时调用 {@link #handleNotFound(String, DefaultContext)} 决定行为；</li>
 *   <li>查询成功时写入 {@link #getResultKey()} 对应的上下文 key，并调用 {@link #afterSuccess(String, Object, DefaultContext)}。</li>
 * </ol>
 *
 * @param <T> 实体类型
 */
public abstract class AbstractSelectorNode<T> extends CPNodeComponent {
     ...
     */
    protected String readMode(DefaultContext context) throws CPReturnException {
     ...
     */
    protected abstract T doSelect(String mode, DefaultContext context) throws Exception;
     ...
     * 子类可以选择：
     * <ul>
     *   <li>调用 {@link #businessError(DefaultContext, String)} 返回业务错误；</li>
     *   <li>调用 {@link #argsError(DefaultContext)} 将其视为参数错误；</li>
     *   <li>什么都不做，表示“未找到但不视为错误”。</li>
     * </ul>
     */
    protected abstract void handleNotFound(String mode, DefaultContext context) throws CPReturnException;

    /**
     * 查询成功后的回调，可用于补充写入上下文或记录日志。
     * 默认空实现。
     */
    protected void afterSuccess(String mode, T entity, DefaultContext context) throws CPReturnException {
        // no-op
    }
}
