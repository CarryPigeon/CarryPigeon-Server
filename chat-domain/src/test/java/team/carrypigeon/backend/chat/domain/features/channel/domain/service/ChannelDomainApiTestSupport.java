...
/**
 * 频道应用测试支撑。
 * 职责：为频道领域服务契约测试提供固定时间、内存仓储替身和最小装配入口。
 * 边界：仅供当前测试包使用，不承载正式业务规则。
 */
final class ChannelDomainApiTestSupport {

    static final Instant BASE_TIME = Instant.parse("2026-04-24T12:00:00Z");

    private ChannelDomainApiTestSupport() {
    }
    ...
    static final class TestContext {
        ...
        ChannelQueryDomainApi createQueryService() {
        ...
        ChannelLifecycleDomainApi createLifecycleService() {
            return createLifecycleService(new NoopTransactionRunner());
        }

        ChannelLifecycleDomainApi createLifecycleService(TransactionRunner transactionRunner) {
            return new ChannelLifecycleDomainApi(
                    channelRepository,
                    channelMemberRepository,
                    channelInviteRepository,
                    channelBanRepository,
        ...
        ChannelGovernanceDomainApi createGovernanceService() {
            return createGovernanceService(new NoopTransactionRunner());
        }

        ChannelGovernanceDomainApi createGovernanceService(TransactionRunner transactionRunner) {
        ...
        ChannelApplicationFlowDomainApi createApplicationFlowService() {
            return createApplicationFlowService(new NoopTransactionRunner());
        }

        ChannelApplicationFlowDomainApi createApplicationFlowService(TransactionRunner transactionRunner) {
            return new ChannelApplicationFlowDomainApi(
                    channelRepository,
                    channelMemberRepository,
                    channelInviteRepository,
                    channelBanRepository,
    ...
    /**
     * `InMemoryChannelInviteRepository` 测试替身。
     * 职责：隔离外部依赖，使测试只验证当前契约边界。
     */
    ...
    /**
     * `InMemoryChannelBanRepository` 测试替身。
     * 职责：隔离外部依赖，使测试只验证当前契约边界。
     */
