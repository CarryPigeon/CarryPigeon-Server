...
- 使用 ECC + AES-GCM 组合：
  - 握手阶段使用 ECC 公钥交换 AES 会话密钥；
  - 业务阶段使用 AES-GCM 对 JSON 业务数据进行加密。

握手流程简要：

1. 客户端发送 `CPECCKeyPack { id, key }`，包含 ECC 公钥；
2. 服务端生成 AES 会话密钥，使用客户端公钥加密后构造 `CPAESKeyPack { id, sessionId, key }` 发送给客户端；
3. 双方保存对称密钥与 `sessionId`，进入加密通信阶段。
