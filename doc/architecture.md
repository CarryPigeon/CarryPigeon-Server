...
1. **握手阶段**
   - 服务器在启动时加载或生成 ECC 密钥对（公钥通过安全途径分发给客户端，不在握手报文中传播）；
   - 客户端本地生成 AES 会话密钥，用服务器 ECC 公钥加密后通过 JSON 发送给服务器；
   - 服务器使用 ECC 私钥解密得到 AES 密钥，写入当前连接的会话状态，并回发一条 `route="handshake"` 的加密通知表示握手成功。
