...
/**
 * 验证基础设施门面的最小契约。
 * 职责：确保上层模块可通过一个简洁入口访问 ID、时间、JSON、插件配置等固定基建能力。
 * 边界：不验证各基础能力内部行为。
 */
@Tag("unit")
class InfrastructureBasicsTests {
    ...
    void accessors_createdWithProviders_returnSameProviders() {
        ...
        PluginConfigurationProvider pluginConfigurationProvider = new PluginConfigurationProvider(new PluginProperties());

        InfrastructureBasics infrastructureBasics = new InfrastructureBasics(
                idGenerator,
                timeProvider,
                jsonProvider,
        ...
        assertSame(pluginConfigurationProvider, infrastructureBasics.plugins());
    }
}
